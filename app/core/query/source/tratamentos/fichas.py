...
class TratamentoFichas:
    ...
    def __init__(self, leitura: list):
        ...
        print(f'dict: {type(self.df_tratado)}')
        # for c, v in self.df_tratado.items():
        #     print(f'{c} : {v}')
        for item in self.df_tratado.items():
            print(item)


    def tratar(self, leitura: list):
        linhas_limpas = [linha.replace('\xa0', '\t').replace('\n', '\t') for linha in leitura]
        valores = [linha.split('\t') for linha in linhas_limpas]
        for valor in valores: print(valor)


        dicio = dict(enumerate(valores))
        return dicio
