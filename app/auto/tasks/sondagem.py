...
class Sondagem:
    ...
    def _obter_tabela_do_elemento(elemento: WebElement) -> dict[str | Any, list[Any]]:
        ...
        cabecalhos = ['Composição', 'Série', 'Turno', 'Código', 'Turma', 'Horário', 'Funcionamento', 'Sala',
        ...
        tabela_turmas = {cabecalho : [] for cabecalho in cabecalhos}
        ...
        for linha in todas_linhas :
            ...
            if len(linha_dados) >= len(cabecalhos) :
    ...
    def _resumir(self) -> dict[str, int | str]:
        ...
        dicionário = self._obter_tabela_do_elemento(elemento)

        resumo["Turmas"] = dicionário["Turma"]
        resumo["Composições"] = list(set(dicionário["Composição"]))
        resumo["Turnos"] = list(set(dicionário["Turno"]))
        resumo["Tipos"] = list(set(dicionário["Tipo Turma"]))
        resumo["Excedente autorizado"] = sum([int(valor) for valor in dicionário["Excedente Autorizado"]])
        resumo["Capacidade física"] = sum([int(valor) for valor in dicionário["Capacidade física"]])
        resumo["Capacidade legal"] = sum([int(valor) for valor in dicionário["Capacidade legal"]])
        resumo["Capacidade Total"] = resumo["Capacidade física"] + resumo["Excedente autorizado"]
        resumo["Efetivados"] = sum([int(valor) for valor in dicionário["Efetivados"]])
        resumo["Vagas disponíveis"] = sum([int(valor) for valor in dicionário["Vagas"]])
