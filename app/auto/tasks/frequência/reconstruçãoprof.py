            ...
            ########################################
                #RESTANTE DO CÓDIGO... A PARTIR DAQUI, O CÓDIGO FUNCIONA (boa parte, pelo menos. Não consegui testar tudo ainda, porque it keeps crashing
                        ...
                        if len(dias_pendentes) > 0 :
                            for dia_pendente in dias_pendentes :
                                ...
                                pontinhos_alvos = [ponto for ponto in pontinhos if
                                                   ponto.get_attribute('data-matricula') in self.ausentes_na_data[
                                                       'Matrícula'] and dia_pendente.get_attribute('data-canonica') ==
                                                   self.ausentes_na_data['Data canonica']]
                                ...
                                for ponto in pontinhos_alvos :
                                    ponto.click()
                                    print(f'Clicando no pontinho de {ponto.get_attribute('data-matricula')}')
                            ...
                            print(f'Lista de dias pendentes vazia. Escapando iteração e passando para a próxima.')
